...
apikey = "5P0AgfObywJQQpbpW8Ok2CP4p-LG24d-qS5VNOnW0Qxc"
url = "https://api.us-south.text-to-speech.watson.cloud.ibm.com/instances/cc12af60-9f9d-444d-a571-e9ad4ee72a23"
#setup service
...
def on_message(self, msg):
    ...
    if "results" in data:
        ...
        #added code to convert the voice to text file in .txt format
        with open("text.txt", "w") as ws:
        
         ws.writelines(data['results'][0]['alternatives'][0]['transcript'])
        #this is chtabot authenticator
        authenticator = IAMAuthenticator('YCYgB86mYnoGSVNi8egVqoG7Weg7iBvgg_A545Gf1n0W')
        assistant = AssistantV2(
        version='2021-07-08',
        authenticator=authenticator
         )
        assistant.set_service_url('https://api.eu-gb.assistant.watson.cloud.ibm.com/instances/d7cd7375-1acc-4280-8f76-e4bf87b7432a')
        assistant.set_disable_ssl_verification(True)          
         #send text to chatbot
        output = assistant.message_stateless(
         assistant_id='06ea24e8-e7ab-4216-b4a3-e240762c5f6f',
         input={
          'message_type': 'text',
          'text': txt
          }
        ).get_result()   
        #get the text from chatbot
        answer = json.loads(json.dumps(output, indent=2)) 
        print(answer["output"]["generic"][0]["text"])
        chatpot_res = answer["output"]["generic"][0]["text"]
        #set the the voice TTS and wrtie it to a mp3 file
        res = tts.synthesize(chatpot_res, accept= 'audio/mp3', voice = "en-US_HenryV3Voice").get_result()
